...
| 12 | Monitoring | ⚠️ **in progress** | 2026-08-15 | **First exit criterion's prerequisite landed: node/ZFS/smartctl exporters now exist on all four hosts, not just maxdata.** `brink-server` (node + `zfs-prometheus-exporter` on `main` + smartctl on the NVMe), `winkel-pi` (node + smartctl — worth it specifically there, its root disk sits behind the ASM1153 USB-SATA bridge already documented as having dropped under write load once; no ZFS, it has no pool), `ionos` (node only — ext4 on virtio, no ZFS and no SMART data to read). ⚠️ **Deliberately not opened on each host's LAN/public firewall.** Ports land on `networking.firewall.interfaces.${tailscale.interfaceName}` only (mirroring `k3s-base.nix`'s existing pattern for the k3s ports), because an in-cluster Prometheus reaches these hosts at their Kubernetes `INTERNAL-IP`, which D3 already puts on `tailscale0` — not via each host's LAN trust, which Phase 8's decision log already flagged as possibly not what makes scraping work on maxdata, and which must not become a dependency here. On ionos specifically this also avoids the global `allowedTCPPorts`, which covers the public `ens6` NIC. All four hosts verified with `nix eval` — `assertions` empty, `services.prometheus.exporters.node.enable` true, and a full `system.build.toplevel` derivation evaluates on every host, maxdata included. **Not yet done, and not attempted this session:** actually scraping them (that's Prometheus scrape config, `homelab-k8s`/Pulumi, out of scope here), cross-site blackbox probes, per-site ntfy alert *routing* for anything beyond the certs/ingress rules Phase 9 already wired, the external dead-man's switch on ionos, and turning `zfs.nix`'s `zfs-health-check` TODO (and brink-server's still-absent `zed` config) into a real ntfy notification. maxdata's own `monitoring.nix` untouched — its `vmbr0` blanket-trust cleanup is a separate open item, not attempted here |
...
- ✅ Node, ZFS and smartctl exporters on all four hosts (maxdata already had all
  three: 9100, 9134, 9116 — `hosts/nixos/maxdata/monitoring.nix:107-126`).
  **2026-08-15: the other three now have them too** —
  `hosts/nixos/brink-server/monitoring.nix` (all three, `main` pool),
  `hosts/nixos/winkel-pi/monitoring.nix` (node + smartctl, no ZFS pool),
  `hosts/nixos/ionos/monitoring.nix` (node only, no ZFS/SMART on a virtio VPS
  disk). ⚠️ Code-complete and `nix eval`-verified only — not yet deployed or
