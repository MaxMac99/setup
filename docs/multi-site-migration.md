...
| 10 | Workloads and bootstrap | ✅ done | 2026-08-12 | **Paperless is fully restored and live; UniFi is deployed but not restored.** ⚠️ **The phase was much smaller than its own text implied** — steps 1–7, 9 and 10 were already running as a side effect of Phases 8/9, leaving only Paperless, UniFi and two decisions. ⚠️ **Three of the four artefacts the phase named do not exist as described**: `postgres-all.sql` is really five gzipped per-database dumps, the backup set is split across the Mac and maxdata with **neither complete**, and the UniFi export is a UniFi **OS** backup rather than a Network `.unf`. Its `pulumi up --target` instruction is also actively wrong and was superseded by the `--exclude` finding. **Paperless:** 730 documents restored into CNPG, media found **already in place** (NFS on `tank` was never touched by the rebuild) and reconciled against the dump file-by-file — **0 missing, 724 pre-existing orphans**; migrations reported *"No migrations to apply"*, confirming the dump matched the deployed image exactly; index rebuilt 730/730 and **search returns 157 real hits**; `dms.mvissing.de` serves a production certificate verified with `curl` **without `-k`**. The Authentik OAuth2 provider was created through **`ak shell`**, copying the working Grafana one, because `authentikApiToken` is dead like the OAuth pairs. ⚠️⚠️ **The find that mattered: `sub` is installation-scoped**, so the restored `socialaccount` rows were dead and the first SSO login would have created a new user owning **none** of the 730 documents — with no way back, since regular login is disabled and every restored password is Django-*unusable*. A working login onto an empty archive: Phase 8's Home Assistant wizard, one app later. Fixed **before** first start by reading the new subject out of Authentik rather than deriving it. ✅ **SSO login confirmed on 2026-08-08 at 10:57, and confirmed as the right outcome**: no stray account, `max` owns all 730 documents, and the *restored* social link's `last_login` advanced — proving the pre-set subject matched. Getting there needed two more fixes, both worth reading: the callback lives under `/accounts/oidc/`, and creating the provider via `ak shell` left `grant_types` **empty**, which Authentik returns to the *client* as `invalid_request`. **UniFi:** workload serving on `192.168.178.243`, both PVCs bound on maxdata; the `.unifi` restore and re-adoption are UI work. Also closed: the `nfs` StorageClass "gap" was **not a gap** (see the log — an NFS provisioner was nearly added to fix nothing), and the six-secrets criterion is **withdrawn**, having been written against a premise §1.3 disproved in Phase 1. ✅ **Phase closed 2026-08-12.** The browser SSO login and the UniFi restore both closed on the 8th, above. The last criterion was **split on measurement**: Brink device discovery is genuinely done — **857 entities, 105 devices, 36 integrations** in the live registries, which is *above* the archived original's 792 and settles that the rebuilt smart home is real rather than another wizard install. **Matter commissioning is withdrawn**, there being no Matter devices to commission yet. ⚠️ **That half was measured rather than taken on report, and the measurement disagreed**: the fabric store holds `vendor_info` and **zero** node keys, and HA's device registry carries **no `matter` identifier** at all — so the integration is loaded, the pod is `Running`, and nothing has ever been paired to it. Two independent sources, because "config entry present and pod healthy" reads exactly like done. Same family as `enabled: false` rewrites and UniFi's `adopted: true`. Carried to Open items, not dropped |
| 11 | Backups that actually exist | ✅ **done** | 2026-08-16 | **The NixOS-side items (2, 3, 4, 5) are done and verified live; item 1 (CNPG WAL archiving) declined 2026-08-16 — the Storage Box it depended on isn't S3-compatible and no bridge was worth the added surface at this scale; item 6 (`fast` redundancy) is a hardware decision, deferred and carried forward.** Sanoid gained coverage it never had — `tank/daten-familie/max/michael/anna` and `tank/k8s` (folding in the NFS-tier item, since Paperless media lives underneath it) — templated in `SMB_SETUP.md` since Phase 8 but never wired into `zfs.nix` until now. `sanoid --monitor-snapshots` reports clean on both maxdata and brink-server. brink-server → maxdata replication (item 2) landed as a **pull**, not a push: a `syncoid-remote` account on brink-server, delegated by `zfs allow` to exactly `bookmark,hold,send,snapshot,destroy,mount` on `main/k8s` — no root SSH (`PermitRootLogin=no` already), no sudo — and a narrow-recipient `secrets/backup.yaml` (maxdata only, like `kubeconfig.yaml`'s pattern) holding the pull key. ⚠️⚠️ **Item 5 (syncoid target retention) looked fixed and was not, and the second attempt is the one worth remembering.** The first version added a sanoid `backupTarget` template (`autosnap=false, autoprune=true`) to prune the targets — but sanoid only ever recognises its **own** `autosnap_*_{hourly,daily,...}` naming, and syncoid (absent `--no-sync-snap`) mints a uniquely-named marker snapshot on the *source* every run. The source stays lean because syncoid cleans up its own old markers there, but the copies already replicated to the target are invisible to sanoid's pruning and never leave — 26 minutes of `sanoid --cron` only pruned 86 of ~7000 snapshots on `tank/fast-backup/k8s`, the 86 that matched sanoid's naming. Real fix, found only by triggering the pull live rather than trusting `nix eval`: brink-server gets its own sanoid on `main/k8s` (it had **zero** point-in-time recovery of its own until now) so both source datasets have a real snapshot schedule, `--no-sync-snap` makes syncoid sync from those instead of inventing new unprunable ones, and `localTargetAllow` gained `destroy` — the module's default omits it, so syncoid's own stale-marker cleanup failed `permission denied` on every run until this. A one-time bulk `zfs destroy` cleared the historical backlog once the forward-going leak was actually closed: `tank/fast-backup/k8s` **13 690 → 86** snapshots, `tank/brink-backup/k8s` clean from the start. `zfs receive` also does not create a missing *parent* dataset (only the leaf) — `tank/brink-backup` did not exist and the first live pull failed until a oneshot `zfs create -p` was added. **Restore verified for real, not assumed**: cloned a replicated snapshot on maxdata (not brink-server) and read genuine Home Assistant data out of it — `automations.yaml`, `home-assistant_v2.db`, `.storage` — with **877 entities**, plausibly above the 857 recorded weeks earlier as devices were added. Test clone destroyed afterward; nothing left mounted |
| 12 | Monitoring | ✅ **done** | 2026-08-16 | ⚠️⚠️ **The phase's own "not started" status was wrong — the Pulumi/`homelab-k8s` side is already substantially deployed**, and finding that out live cost one crash-looping unit. `monitoring` namespace, running 2+ days before this session touched anything: Grafana, Loki (+ `loki-canary`), Prometheus server + Alertmanager + `kube-state-metrics` + pushgateway, Tempo, ntfy, unpoller, and an **Alloy DaemonSet already on all four nodes** — so log shipping already exists estate-wide, independent of maxdata's own host-level Alloy in `monitoring.nix` (which is therefore now running **twice** on maxdata; not fixed this session, flagged here). **`prometheus-prometheus-node-exporter` is a hostNetwork DaemonSet on brink-server, winkel-pi and ionos already**, with an explicit `nodeAffinity` (`kubernetes.io/hostname NotIn [maxdata]`) written specifically to exclude maxdata, because maxdata is the one host that predates the DaemonSet and keeps its own. First attempt at this session's work added a *second* node-exporter on all three DaemonSet-covered hosts, mirroring maxdata's `monitoring.nix` — deployed to brink-server, and it crash-looped immediately: `listen tcp 0.0.0.0:9100: bind: address already in use`, `systemctl --failed` showing exactly what the affinity rule already knew. **Caught before winkel-pi or ionos were touched**, by checking `ss -tlnp` and tracing the pid's cgroup back to a `kubepods` slice rather than assuming the failure was something else. ✅ **Corrected and now live**: `brink-server` and `winkel-pi` carry only `zfs-prometheus-exporter` (`main` pool) / smartctl, neither of which exists anywhere in-cluster; `ionos` ended up with nothing to add (no ZFS, no SMART-capable virtio disk, node already covered) and its `monitoring.nix` was deleted rather than shipped empty. Ports scoped to `networking.firewall.interfaces.${tailscale.interfaceName}` only, mirroring `k3s-base.nix`'s pattern for the k3s ports — an in-cluster Prometheus reaches these hosts at their Kubernetes `INTERNAL-IP`, which D3 already puts on `tailscale0`, not via each host's LAN trust. **Deployed and verified live 2026-08-15**: `brink-server` (`git fetch`→fast-forward confirmed→`reset --hard`→`switch`, zero failed units, `9134`/`9116` active, `9100` correctly still owned by the DaemonSet's `node_exporter` pid in a `kubepods.slice` cgroup) and `winkel-pi` (`dry-activate` first per the scripted-networking caution — only `firewall.service` reload and a `systemd-udevd` restart, no networking unit — then `switch`, zero failed units, `9116` active). Both exporters confirmed **reachable end-to-end over the overlay** with `curl` from maxdata to `100.64.0.2:9134`/`9116` and `100.64.0.3:9116`, all `200`. All four k3s nodes stayed `Ready` throughout both deploys. ✅ **External dead-man's switch also done, same session, and it changed repos mid-work.** Original plan was a host-level systemd timer on ionos with its own cluster-health check — reconsidered before deploying anything (see the phase text below for why: an in-cluster `CronJob`'s mere ability to run is a strictly better, simpler health signal, so this belongs in `homelab-k8s`, not here). Reverted the uncommitted NixOS-side work cleanly, then landed `monitoring/deadmans-switch.ts` in `homelab-k8s` — a `curlimages/curl` CronJob every 5 minutes, no `nodeSelector` (losing one node, even ionos, must not trip this), pinging a healthchecks.io URL held in Pulumi's own secret config. Deployed with `pulumi up` and proven live with a manual `kubectl create job --from=cronjob`, which pinged successfully. ⚠️ Not yet fire-tested against a real outage — see 12.1. ✅ **"All four hosts scraped" closed same session**: `monitoring/prometheus.ts` gained `brink-server-zfs`, `brink-server-smartctl`, `winkel-pi-smartctl` jobs, targeting each host's **overlay** address (`100.64.0.2`/`.3`), not its LAN IP — maxdata's existing three jobs only work at its LAN IP because the Prometheus pod is itself pinned to maxdata (same-node traffic); brink-server and winkel-pi are different nodes, and maxdata does not accept their subnet routes (only subnet routers do, D3), so the overlay address is the only one reachable from where Prometheus actually runs. All three confirmed `health: up` in `/api/v1/targets` within one scrape interval of `pulumi up`. ⚠️ Minor: the commit that landed this was mistitled (copy-pasted the *previous* commit's message) and already pushed — left as-is per this repo's own rule against rewriting pushed history, noted here instead. ✅ **"ZFS degradation notification" also closed the same way as the dead-man's switch — reused, not built.** `zfs.nix`'s `# Add notification here` TODO and brink-server's missing `zed` config are superseded rather than fixed: `ZfsPoolNotOnline` (`zpool_state` is one-hot per pool, `state="online"` reading 0 means some other state) and `ZfsVdevErrors` (`increase()` on the exporter's own read/write/checksum counters, catching errors redundancy already absorbed) both query `zfs-prometheus-exporter` — already scraped on both hosts — through the same Alertmanager → ntfy pipeline as `CertificateExpiringSoon`. Verified loaded (`health: ok`, `state: inactive`) via the Prometheus rules API, not assumed from a clean deploy. Nothing new was deployed to either host for this. ⚠️⚠️ **A real, self-inflicted ~40-minute cluster incident happened attempting cross-site blackbox probes — full account in the phase's own body text below.** Short version: a bad `tailscaled -debug` flag crash-looped `tailscaled` on maxdata, which silently emptied its flannel VXLAN FDB (a chicken-and-egg bind-before-interface-exists bug, unrelated to and worse than the direct symptom), breaking all inbound cross-node pod traffic to maxdata — surfaced only because the user reported `unifi` devices going offline. Fixed with `systemctl restart k3s` on maxdata; fully verified clean afterward. Cross-site blackbox probes (ICMP latency/loss, Grafana dashboard 7587) then landed without incident, and along the way found and fixed the identical incident masquerading as a "second" DNS defect — corrected in-place once retested, not left as two separate findings. ✅ **maxdata's duplicate Alloy fixed too**: it shipped the systemd journal to Loki twice (its own host-level Alloy and the in-cluster DaemonSet, which covers every node including maxdata with no exclusion); removed the journal half, kept Samba log shipping, which the DaemonSet doesn't cover. ✅✅ **Both remaining exit criteria closed for real, not just deployed.** ntfy phone delivery: created a read-only `max` user scoped to the `alerts` topic (`ntfy access max alerts read`), confirmed with two manual test publishes reaching the phone. Then, reversing the earlier decline, the ZFS degradation criterion was actually fire-tested live: `zpool offline`d one `tank` `raidz1-0` disk (4-disk RAIDZ1, zero data risk) — pool went `DEGRADED`, `ZfsPoolNotOnline` transitioned `pending` → `firing` at exactly the `for: 5m` mark, and the notification arrived on the phone. `zpool online`d immediately after (no resilver needed), pool back `ONLINE`. The **resolved** notification took noticeably longer than expected; traced through ntfy's own request logs (not assumed) to Alertmanager's `group_interval: 5m`, which batches resolved notifications on the next scheduled flush rather than sending them immediately on resolve — arrived exactly on that schedule once understood, not a bug. **Every 12.1 exit criterion is now closed, with real evidence rather than a clean deploy, for all four.** Remaining open items are outside the phase's own exit criteria: per-site ntfy alert *routing* beyond the single `alerts` topic (not needed at this scale, not attempted), and the `homelab-k8s` branch-protection bypasses already noted below.

⚠️⚠️ **A real, self-inflicted ~40-minute cluster incident happened attempting cross-site blackbox probes, on 2026-08-16 — full account kept here rather than split across commits.** Original plan: expose `tailscaled`'s native per-path traffic counters (`direct_ipv4`/`direct_ipv6`/`derp`) for the direct-vs-relay signal, via `tailscaled -debug=<addr>`, added to the shared `overlay-client.nix` (all four hosts). **First deploy attempt bound the debug server to the host's own overlay address** (`-debug=100.64.0.5:9002` on maxdata) — fatal: at process start the debug server binds *before* tailscaled has brought up `tailscale0` and assigned that address to it, a chicken-and-egg `bind: cannot assign requested address`, and `tailscaled` crash-looped. **maxdata's Kubernetes node went `NotReady`**, not just its overlay — confirmed via `unifi` namespace events (`NodeNotReady`, then MetalLB re-announcing the `unifi` LoadBalancer from winkel-pi). Fixed in ~2 minutes by rebinding to `0.0.0.0` (firewalled to the overlay interface only, same pattern as every other exporter here) and redeploying — `tailscale status` confirmed all peers `direct` again. **Then the debug server itself turned out not to expose what was needed at all**: `/debug/metrics` (the actual HTTP path — not `/metrics`) is Go runtime + tailscale internals, but *not* the `tailscaled_{in,out}bound_bytes_total{path=...}` counters, which come from `tailscale metrics` reading the local Unix socket API, unreachable over the debug HTTP server. Decision: drop `-debug` entirely rather than build a custom exporter for it — reverted cleanly from all four hosts (`git diff` empty), redeployed to maxdata to confirm.

⚠️⚠️ **The real damage was quieter and found only because the user reported UniFi devices going offline afterward.** `flannel.1` is a VXLAN device bound to `tailscale0` (`vxlan id 1 local 100.64.0.5 dev tailscale0`). Tearing `tailscale0` down and recreating it (twice, across the broken and fixed `-debug` deploys) left flannel's own FDB/neighbor table on maxdata **completely empty** — `ip neigh show dev flannel.1` and `bridge fdb show dev flannel.1` returned nothing at all, confirmed against winkel-pi's identical commands showing all three peers correctly populated. This silently broke **all inbound pod-network traffic to maxdata from every other node** — not just the `unifi` VIP MetalLB had already failed over to winkel-pi, but literally any cross-node packet destined for a maxdata pod, because return traffic had no route out. Diagnosed by elimination: `unifi` Service iptables/NAT rules on winkel-pi were correct and pointed at the right pod IP, `externalTrafficPolicy: Cluster` was as expected, but a *direct* `ping` from winkel-pi to the pod IP itself (bypassing the Service entirely) showed 100% loss — proving it was flannel routing, not Service NAT. Fixed with `systemctl restart k3s` on maxdata, which forced flanneld to fully re-provision its VXLAN state; FDB repopulated immediately, `ping` 0% loss, VIP returns `200`, all four nodes `Ready`, and a full cluster-wide pod sweep showed nothing abnormal except the two `deadmans-switch` runs that correctly errored during the outage window. ⚠️ **This is now a known hazard for this repo, not just an incident**: any future `tailscaled` restart on a host running k3s (i.e. every host in this fleet) risks silently breaking inbound pod-network traffic to that host until something forces flannel to resync — `overlay-mtu.service` already has a documented version of this same class of bug (re-runs on device recreation); flannel does not, and nothing here currently re-triggers it. Worth a Phase 13 or later item: either a systemd unit that restarts k3s (or just re-syncs flannel) after `tailscaled.service` restarts, or an alert on empty/stale `flannel.1` FDB state.
...
| **`trustedInterfaces = ["vmbr0"]` moved to Phase 12** | `hosts/nixos/maxdata/networking.nix` deferred this to Phase 8 on the grounds that Phase 8 "wires NFS up against a real consumer". It does not: Paperless, Time Machine and Prometheus are all deployed in Phases 10/12, so removing the blanket trust now would be untestable — the situation Phase 6 deferred it to avoid. Established meanwhile: **Samba is safe** (`smb.nix` sets `openFirewall` for Samba and Avahi independently), **the exporters are not** (`node` 9100 and `smartctl` 9116 have no `openFirewall`; `zfs-prometheus-exporter` 9134 does), and **k3s is already covered** (`k3s-cluster.nix:157-159` opens 6443 on `lanInterface` explicitly, written for exactly this). ⚠️ Unsettled: Prometheus scrapes from a *pod*, so packets reach `192.168.178.2:9100` via `cni0`/`flannel.1`, not `vmbr0` — meaning the blanket trust may never have been what made scraping work, and adding the ports to `allowedTCPPorts` may not be sufficient either. Measure it once Prometheus exists. ⚠️ **There are now two of these, not one — 2026-08-08.** brink-server gained `trustedInterfaces = ["eno1"]` for Music Assistant, which streams audio from its own HTTP server and so needs the *players* to connect inbound, and whose AirPlay/Sonos/Chromecast providers open **random** ports in both directions — upstream states no port list is correct. So this cleanup can no longer be "delete the blanket trust"; brink-server needs a different answer or an explicit exception. What it exposes there was measured: kubelet `10250` (client-cert authenticated), the node-exporter/MetalLB metrics ports, rpcbind, two NodePorts, matter-server and Home Assistant — but **not etcd**, which binds only to `127.0.0.1` and `100.64.0.2` | Phase 8, 2026-08-07 · extended Phase 10, 2026-08-08 |
...
| **Restoring UniFi does not re-adopt anything, and the UI cannot fix it** | ⚠️ **"Upload the backup, then re-adopt via the UI" — the second half is wrong.** Restoring brought all four devices back with `adopted: true`, but each still held the **old** controller's inform URL and went on posting to an address that no longer exists, so they showed offline and **never appeared under Pending Adoption**. ⚠️ Nor does L2 discovery rescue it: the pod is not `hostNetwork`, so a device's broadcast to `255.255.255.255:10001` cannot reach it — the `10003/UDP` port on the LoadBalancer carries unicast only. The old address was **`192.168.178.13`**, recoverable only from `pulumi-stack-pre-multi-site.json`, which Phase 13 deletes — hence recording it here and in `apps/unifi.ts`. Fixed by SSHing each device and `set-inform http://192.168.178.243:8080/inform`, using the credentials the backup itself restored (`db.setting.find({key:"mgmt"})`). All four re-adopted **within ~30 s and with no manual adoption step**, because the restore still held their auth keys. ⚠️ **Verify with `last_seen`, never with `adopted`** — the restore sets `adopted: true` on every device, so it reads as success before a single packet has arrived; `last_seen` was still the backup's own timestamp on the three untouched devices while the test device's had jumped to *now*. That contrast is the only thing that distinguished working from restored-but-dead | Phase 10, 2026-08-08 |
| **Creating Authentik objects through `ak shell` skips serializer defaults** | ⚠️ **The cost of the `ak shell` route, and it surfaces as a client bug.** `OAuth2Provider.objects.get_or_create()` applies *model* defaults only — so `grant_types` came out **empty**, because it is populated by Authentik's DRF serializer, which the UI and REST API go through and the ORM does not. Authentik then logs `Invalid grant_type for provider` and returns `error=invalid_request` **to the callback URL**, so the browser lands on Paperless with a generic OAuth error and nothing points at Authentik. `access_token_validity` (`hours=1` vs the UI's `minutes=5`) and `refresh_token_threshold` fell through the same way. ⚠️ **"Copy the working provider" is not enough if you only copy the fields you thought of** — the fix was to diff the two objects **field by field** with `_meta.get_fields()` and assert the remaining difference set is empty apart from name, client credentials and redirect URI. Do that before trusting anything built this way | Phase 10, 2026-08-08 |
| **Paperless's OIDC callback is under `/accounts/oidc/`, not `/accounts/`** | The setup notes in `apps/paperless.ts` documented `https://dms.mvissing.de/accounts/authentik/login/callback/`, and Authentik rejected it with *"The request fails due to a missing, invalid, or mismatching redirection URI"*. django-allauth namespaces the `openid_connect` provider under **`/accounts/oidc/<provider_id>/login/callback/`**, where `<provider_id>` is the `id` field inside `PAPERLESS_SOCIALACCOUNT_PROVIDERS` — `authentik` here, which is why the wrong form looks so plausible. ⚠️ **Do not derive it twice from the same guess**: the URLconf was read out of the running app (`get_resolver()`), and then the value Paperless *actually sends* was captured by POSTing the login form and parsing `redirect_uri` out of the resulting authorization URL. Those are two independent sources, and both had to agree before the provider was changed | Phase 10, 2026-08-08 |
...
| **D18** | **The tailnet's exit node is ionos** — `--advertise-exit-node` there, routes approved once on the control server, clients (Mac, iPhone) select it **on demand** | **Added 2026-09-09.** Motivation: the overlay so far reaches the estate but offers no egress — on café/hotel Wi-Fi a laptop can reach `*.mvissing.de` and the k3s API but browses through whatever hostile network it sits on. An exit node makes the overlay a *true VPN*. **Why ionos and not a home host:** it is the only host with a fixed public IP (stable exit address, D2), the only one not behind CGNAT on a consumer uplink, and the roaming resolver (D15) runs on it — so an exit-node client's DNS rides the same box instead of crossing the overlay a second time. Exiting via a home host would be a *different* feature ("appear from home"), couples the VPN to a DG uplink, and presents a changing address; Headscale supports several exit nodes and the client picks, so a home one can be added later without redesign. ⚠️ **Mechanics are the same two-step as the subnet routers:** advertise (`--advertise-exit-node`, `modules/system/overlay-client.nix`) then approve (`headscale nodes approve-routes --identifier <ionos-id> --routes 0.0.0.0/0,::/0`) — skip the second and every client reports healthy with no exit node offered, the same silent no-op 3.2 documented. ⚠️ **The default routes must never be installed as a side effect of `--accept-routes`** — client-side, a `0.0.0.0/0` route lands in table 52 only on explicit exit-node selection, never with accepted routes; that is the load-bearing assumption that lets the subnet routers keep `--accept-routes` while ionos advertises the default routes, and it is the shape of failure 3.6.1 already produced once. Verify after approval: no `default` in table 52 on brink-server, winkel-pi or maxdata, and their default routes still from DHCP. ⚠️ **ionos config deltas are deliberately nil beyond the flag:** forwarding sysctls already on via k3s (`useRoutingFeatures` moves `none` → `both`, which the kernel had already been told), exit-node SNAT is installed by tailscaled itself, no new ports in the IONOS firewall (exit traffic is outbound WireGuard on the existing endpoint), and userspace memory cost is nil on the 1.8 GiB box. ⚠️ **Client selection is per-device, per-connection and stays manual on purpose** — macOS menu bar / iOS app — because "always exit via the VPS" is wrong at home: the Mac's on-demand rules (except on the Brink/Winkel SSIDs) already keep the mesh off both LANs, and the iPhone follows the same rule. `--accept-dns` pairs with the exit node: with it on, the client resolves through the roaming AdGuard on the exit node itself, which is the D15 behaviour the exit use case wants. ✅ **Deployed and verified 2026-09-09** from maxdata per the standing ionos procedure; routes approved (`headscale nodes approve-routes --identifier 1 --routes 0.0.0.0/0,::/0`, confirmed **Serving (Primary)**); table 52 clean on all four NixOS hosts; end-to-end from the Mac with `--exit-node=ionos.mesh.mvissing.de`: `curl -4 ifconfig.me` → **212.132.82.102** and `curl -6` → **2a02:2479:5c:a00::1**, ionos's exact public pair, reverting cleanly to the local address. ⚠️ **Two findings from the deploy.** (1) **A switch does not apply changed overlay flags to a logged-in node**: `tailscaled-autoconnect` sees BackendState Running and skips `tailscale up`, so `--advertise-exit-node` sat unapplied until a manual `sudo tailscale up` with the module's exact flag set ran once on the box. Saved prefs persist across restarts — which is why reboots never showed this gap — so **any change to `extraUpFlags` needs one manual `tailscale up` per changed host**, or the client reports healthy while the control server never sees the advertisement. (2) k3s failed once during the same switch — it restarted in the same sweep as tailscaled, and `--flannel-iface=tailscale0` needs the interface up — then auto-recovered in ~15 s; expect that blip on any switch that restarts tailscaled, and check k3s afterwards rather than assuming. *(Not a phase of its own — recorded here because every other overlay decision is.)* **Extended 2026-09-13: maxdata is a second exit node** (`hosts.<host>.exitNode`), offering Winkel's residential uplink as egress — the "home one can be added later" above, done. Unlike ionos it is behind CGNAT/DS-Lite, so its exit address is shared and unstable: outbound egress only. Same two-step mechanics; the approval is a separate `approve-routes` for maxdata's own id. |
...
- [x] All applications reachable and authenticating via Authentik
      ...
      the pod **with certificate verification enabled**. ✅ **Browser login
      confirmed 2026-08-08 10:57** — and confirmed as the *right* outcome, not
      merely a successful one: `auth_user` still holds the same **5** rows from
      the dump with **no stray account**, `max` (id 4) owns **730** documents,
      and the restored `socialaccount` row's own `last_login` advanced to the
      same instant — which is what proves allauth matched the **pre-set**
      subject rather than signing up a new user. ⚠️ Two further defects had to
      be cleared first, both in the decision log: the callback URL is under
      `/accounts/oidc/`, and `ak shell` had left `grant_types` empty.
      `PAPERLESS_SOCIAL_AUTO_SIGNUP` is now **False** — it existed only to
      bootstrap that first login.
...
- [x] UniFi devices adopted at Winkel — ✅ **2026-08-08.** Workload on
      `192.168.178.243`, both PVCs bound on maxdata, backup restored, and all
      **4** devices (US-8-150W + 3× UHDIW) checking in. ⚠️ **Re-adoption was
      not UI work and the UI could not have done it** — see the decision log:
      the devices held the old controller's address and never appeared under
      Pending Adoption. ⚠️ Still open: `unpoller` gets **403** on
      `/api/auth/login`; the `unpoller` admin exists in the restored config but
      its credential does not match `unpoller-password` in Pulumi config
- [x] Home Assistant discovering Brink devices — ✅ **2026-08-12**, and
      measured rather than reported. The live registries hold **857 entities,
      105 devices and 36 config entries**: hue 18, icloud 15, hacs 9, shelly 7,
      music_assistant 7, home_connect 5, sonos 4. The entity count is *above*
      the 792 of the original archived in Phase 8, so the rebuilt smart home
      has overtaken what was abandoned — which is the point of the count, since
      Phase 8's failure looked like a working Home Assistant at 63 entities.
- [x] ~~Matter devices commissioned~~ — **withdrawn 2026-08-12, not passed.**
      There are no Matter devices to commission yet, so the criterion assumed
      hardware that does not exist. Same disposition as Phase 6's ARC item and
      the six-secrets criterion above. ⚠️ **Recorded because it was nearly
      closed as done:** `matter-server` is `Running`, the `matter` config entry
      is present, and the fabric store exists — all of which reads as a working
      integration. It is empty. The fabric store
      `/data/658005795383821879.json` has top-level keys `['vendor_info']` and
      **zero** numeric node keys, and HA's device registry contains **no
      `matter` identifier**, so nothing has ever been paired. Two independent
      sources, per the `redirect_uri` rule in 10.3 — one of them alone would
      have been the busybox-`wget` mistake again. ⚠️ **Phase 14 does not
      unblock this and must not be sequenced as if it did**: Home Assistant
      runs `hostNetwork`, so Matter uses brink-server's own IPv6 stack and
      never touches the CNI. Carried to Open items.
...
1. ❌ **CNPG WAL archiving** to object storage — **declined 2026-08-16.** The
   Hetzner Storage Box named here (`u499100.your-storagebox.de`,
   `modules/profiles/personal-ssh.nix:42`) is SFTP/CIFS/WebDAV only, not
   S3-compatible, and CNPG's `barmanObjectStore` needs an S3 endpoint. Bridging
   it (rclone S3 gateway, Hetzner's separate Object Storage product, or a
   self-hosted MinIO) was weighed and rejected — see 11.1. Crash-consistent
   snapshots (items 2–5 below) are the only CNPG recovery path.
...
- [x] ~~CNPG WAL archiving verified; a PITR restore rehearsed into a scratch cluster~~
      — ❌ **declined 2026-08-16.** The premise was wrong: the Hetzner Storage
      Box the item named (`u499100.your-storagebox.de`) is SFTP/CIFS/WebDAV
      only, and CNPG's `barmanObjectStore` needs an S3-compatible endpoint. The
      three ways to bridge that — an rclone S3-gateway sidecar in front of the
      box, Hetzner's separate Object Storage product, or a self-hosted MinIO on
      maxdata's `tank` — were weighed against the value they'd add: the first
      is real but a second moving part with its own credential to manage; the
      second is a new paid resource; the third sits on the same box as a
      sanoid/syncoid target already and adds no real disaster-recovery value.
      **Decision: not worth it at this scale.** Crash-consistent sanoid/syncoid
      snapshots (items 2–5, done) stay the only recovery mechanism for CNPG;
      PITR is out of reach without minute-level granularity that nothing here
      currently needs
- [x] brink-server → maxdata replication running and verified by restoring a PV
      — pull-based (`syncoid-remote` on brink-server, delegated by `zfs allow`,
      no root/sudo); verified live 2026-08-15
- [x] `sanoid --monitor-snapshots` clean for all `tank/daten-*`
      — clean for every monitored dataset on both maxdata and brink-server,
      not just `daten-*`
- [x] A restore has been performed from a real backup, not a snapshot
      — cloned a replicated snapshot on maxdata and read genuine Home
      Assistant data out of it (877 entities); test clone destroyed after

⚠️ Not an exit criterion originally, but should have been: **item 6, `fast`
redundancy, remains open** — a hardware/procurement decision (an extra NVMe or
a mirror), not something code can close. Carried forward rather than dropped.
...
- ✅ **Node exporter is already on every host** — maxdata via its own
  host-level `services.prometheus.exporters.node`
  (`hosts/nixos/maxdata/monitoring.nix:107-126`), the other three via an
  in-cluster `prometheus-prometheus-node-exporter` DaemonSet that already
  excludes maxdata by an explicit `nodeAffinity`. ✅ **ZFS and smartctl,
  which the DaemonSet does not provide, deployed and verified live
  2026-08-15**: `hosts/nixos/brink-server/monitoring.nix`
  (`zfs-prometheus-exporter` on `main` + smartctl on the NVMe),
  `hosts/nixos/winkel-pi/monitoring.nix` (smartctl only — no ZFS pool, but
  worth having given its USB-SATA bridge's write-load history). `ionos` has
  neither ZFS nor a SMART-capable virtio disk, so nothing was added there.
  ⚠️ First attempt duplicated the DaemonSet's node exporter on all three
  hosts and crash-looped on brink-server (`address already in use`) before
  winkel-pi or ionos were touched — see the phase's own Status row for the
  full account.
...
- ✅ **External dead-man's switch — deployed and verified live 2026-08-15,
  but landed in `homelab-k8s`, not here.** First design was a host-level
  systemd timer on ionos with its own `kubectl get nodes`/`/healthz` health
  check, exactly as this bullet originally proposed. **Corrected before
  deploying**: the point of the receiver being external (healthchecks.io) is
  wasted if the *ping source* still depends only on ionos specifically —
  a Kubernetes `CronJob` needs the apiserver, scheduler, kubelet and CNI all
  working *somewhere* in the cluster just to run at all, which is a strictly
  better and simpler health signal than any check this repo could hand-roll,
  and it naturally survives losing any single node (including ionos, one of
  three etcd members) without tripping. `monitoring/deadmans-switch.ts` in
  `homelab-k8s`: a `curlimages/curl` CronJob, `*/5 * * * *`, no
  `nodeSelector`, pings a healthchecks.io URL held in Pulumi's own encrypted
  config (`deadmans-switch-ping-url`) — deliberately **not** sops, since
  `homelab-k8s` has its own secret convention (`Pulumi.default.yaml`) and the
  ping source now lives entirely in-cluster. Deployed with `pulumi up`
  (2 created, 318 unchanged) and proven with a manual
  `kubectl create job --from=cronjob/deadmans-switch`, which completed and
  logged `OK` from `curl`. ⚠️ Ntfy notifications for the phone are a separate,
  later item — this only covers the "everything alerting depends on is down"
  fallback, not day-to-day alert delivery.
- ✅ **Cross-site blackbox probes and the ZFS/smartd/`zfs-health-check`
  notification, both done — see the Status row above for the full account**,
  including the real incident the blackbox-probes work caused and fixed.

## 12.1 Exit criteria

- [x] All four hosts scraped — node metrics via the existing DaemonSet (3
      hosts) + maxdata's native exporter; ZFS/smartctl via the new
      `prometheus.ts` jobs, all confirmed `health: up`
- [x] Cross-site probes graphed — `blackbox_exporter` (ICMP) pinned to
      maxdata, probing brink-server/ionos (cross-site) and winkel-pi
      (same-LAN control), Grafana dashboard 7587 provisioned. `probe_success`
      1 on all three with plausible RTTs. Direct-vs-relay tracking was
      attempted and abandoned — see the phase's Status row
- [x] Simulated cluster outage triggers the external dead-man's switch —
      not simulated, **real**: a genuine ~40-minute cross-node networking
      outage on 2026-08-16 (see the phase's Status row) left two
      `deadmans-switch` CronJob runs in `Error` state, correctly unable to
      reach healthchecks.io while the cluster was degraded, and the next run
      after the fix `Completed` normally. Stronger evidence than a deliberate
      simulation would have been
- [x] A simulated ZFS degradation produces an actual notification, not a
      journal line — **done for real 2026-08-16**, reversing the earlier
      decline. `zpool offline`d one `tank` `raidz1-0` disk (4-disk RAIDZ1,
      zero data risk): pool went `DEGRADED`, `zpool_state` flipped within one
      scrape, `ZfsPoolNotOnline` went `pending` → `firing` at exactly the
      `for: 5m` mark, and the phone notification arrived — full chain proven
      live: exporter → Prometheus rule → Alertmanager → ntfy → phone.
      `zpool online`d immediately after, pool back `ONLINE` with no resilver
      needed, alert cleared. The **resolved** notification took longer than
      expected — confirmed via ntfy's own logs that Alertmanager batches it
      on `group_interval` (5m) from the last group flush, not immediately on
      resolve, and it arrived exactly on that schedule once understood
...
✅ **Phase 10 closed 2026-08-12, so this gate is lifted.** It required Home
Assistant discovering Brink devices and Matter commissioning; the first is done
and measured (857 entities, 105 devices), and the second was **withdrawn** for
want of any Matter devices to commission. The reasoning behind the gate is
unaffected and still worth keeping: rebuilding the cluster underneath an open
hardware-discovery problem means debugging two moving things at once, and
Matter does **not** need this phase — Home Assistant runs `hostNetwork`, so it
bypasses the CNI entirely and already uses brink-server's own IPv6 stack.
⚠️ **So this phase cannot be justified by Matter**, then or now.

⚠️ **Ordering against Phase 11, decided 2026-08-12.** This phase runs *before*
backups, because 14.5 destroys and recreates the cluster and Phase 11's
automation would otherwise be built twice. The cost is that the rebuild happens
with **no backup automation in existence** — so the pre-rebuild capture in 14.5
is not optional bookkeeping, it is the only copy. Phase 1's set is five days
stale and, per Phase 10, split across the Mac and maxdata with **neither half
complete**.
...
### 14.1a Measured 2026-08-12 — the ping half passes on every pair

**All 12 directed pairs carry an overlay MTU of 1400, 0% loss**, measured
in-tunnel with `ping -M do -s 1372` (1372 + 28 = 1400) for 10 packets each, with
`tailscale0` raised to 1400 on all four nodes and reverted to 1280 afterwards.
All four nodes stayed `Ready` throughout.

| from → to | ionos | brink-server | winkel-pi | maxdata |
|---|---|---|---|---|
| **ionos** | — | 0% / 10.8 ms | 0% / 13.3 ms | 0% / 12.2 ms |
| **brink-server** | 0% / 10.8 ms | — | 0% / 9.8 ms | 0% / 5.1 ms |
| **winkel-pi** | 0% / 13.1 ms | 0% / 6.2 ms | — | 0% / 1.1 ms |
| **maxdata** | 0% / 12.2 ms | 0% / 5.4 ms | 0% / 1.2 ms | — |

1400 is not the ceiling; it is a round number above the 1350 the assertion
demands. The physical MTU is 1500 on every host's uplink (`ens6`, `eno1`,
`end0`, `vmbr0`), so with 80 bytes of WireGuard the true ceiling is **1420**.

⚠️ **Raising the MTU is not as cheap as step 1 implies, and the reason is not in
the text.** Every node's `INTERNAL-IP` is its `100.64.0.x` address, so etcd peer
traffic and the apiserver ride `tailscale0` *at host level* — an MTU the path
cannot carry blackholes the control plane, not merely the test. Mitigated by
arming a **dead-man revert first** (`systemd-run --on-active=240 -- ip link set
dev tailscale0 mtu 1280`) on every node before raising anything, the same
pattern Phase 6 used for the networkd bridge. Worth keeping: the SSH control
channel to ionos is its *public* address, and the home hosts are reachable on
their LANs, so none of the recovery path depends on the overlay under test.

✅ **Bulk TCP and the relayed path both done the same day — the gate is closed.**
20 MB of `/dev/urandom` maxdata → brink-server, compared by SHA-256 at both
ends, on **all three paths the pair can actually take**:

| path | DF ping @1400 | 20 MB byte-exact | throughput |
|---|---|---|---|
| direct, native IPv6 (today's steady state) | 0% loss, 5.1 ms | ✅ `c340afd8…` | 294 Mbit/s |
| direct, IPv4 through Brink's DS-Lite CGNAT | 0% loss, 5.7 ms | ✅ `0c7100a4…` | 280 Mbit/s |
| **DERP relayed via ionos** | 0% loss, **23.2 ms** | ✅ `5e2cc219…` | **31 Mbit/s** |

⚠️ **The relayed path works but costs an order of magnitude** — 31 Mbit/s
against 294, and every byte crosses ionos, which has 1851 MB of RAM and no
swap. Relay fallback is therefore survivable rather than benign: it is what
happens when a home uplink flaps, and it silently moves the entire cross-site
pod load onto the one host that must never be put under memory pressure. Worth
an alert in Phase 12 — `tailscale status` reporting `relay` rather than
`direct` for any node pair is a condition nothing currently watches.

⚠️ **Bulk TCP needed a listener and `tailscale0` is a trusted interface on no
host** — ionos trusts `flannel.1`/`cni0`/`flannel-v6.1`/`wg0`, brink-server
`eno1`, maxdata `vmbr0`. Done with a single runtime `iptables -I nixos-fw 1 -i
tailscale0 -p tcp --dport 55555 -j ACCEPT` on the receiver, never in the config,
with `systemd-run --on-active=300 -- systemctl restart firewall.service` armed
first as the dead-man: restarting `firewall.service` re-applies the declarative
ruleset and discards anything ad-hoc, which makes it a better revert than
deleting the rule by hand. Verified 0 rules remaining afterwards.

⚠️ **Two harness failures worth keeping, both of which produced output that
could have been misread as a result.** (1) The first transfer reported
`20 MB in 16 ms` and a receive hash of
`e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855` — **the
SHA-256 of zero bytes**. `nc` on these hosts is **LibreSSL/OpenBSD**, where
`-p` is the *source* port, so the `nc -l -p 55555` listener died instantly and
the sender's connection was refused. 1.25 GB/s across a 5 ms WAN link is the
other tell. **Know the empty-string hash on sight, and sanity-check throughput
against physics.** (2) The first attempt at forcing DERP blocked only IPv6 —
see 14.1c.

### 14.1b Two probes that produced clean-looking numbers and measured nothing

⚠️ **Both were nearly believed.** They are recorded because each returned a
plausible table, and in one case a *uniformly passing* one.

1. **DF-ICMP to a peer's underlay address measures the home routers' firewall
   policy, not path MTU.** Probing each peer's real IPv6 endpoint with
   `ping -6 -M do` returned FAIL on every leg toward a home site and OK to 1500
   on every leg toward ionos. Read as path MTU that says Brink and Winkel cannot
   carry 1280 — **impossible**, since 1280 is IPv6's mandatory minimum and those
   same addresses were passing gigabytes (`tx 40594681884` on brink↔maxdata) at
   that moment. The FritzBox and UDM SE drop unsolicited inbound ICMPv6 echo;
   WireGuard's UDP/41641 flows because those mappings are established. **The
   tell is a failure at 1280**: below the protocol floor, a size result is not a
   size result. In-tunnel ICMP does not have this problem — the routers see only
   encrypted UDP.
2. **`tailscale ping --size` is not a PMTU probe.** It looked ideal: disco pings
   ride the real UDP path rather than the TUN, so they cross the firewalls.
   `--size 1520` returned a **direct reply on every leg**, including
   brink→ionos, where ICMP had independently established the ceiling at 1500.
   A probe that passes above the known physical limit is clamping or padding
   internally, and its clean sweep proves nothing. **Discarded on the strength
   of the calibration, not the result** — had the sweep been run only on the
   unmeasurable legs it would have read as a comprehensive pass.

⚠️ **Generalise: calibrate a new probe against a leg whose answer is already
known, before trusting it on the legs you cannot otherwise measure.** Both
failures were caught that way and neither would have been caught by inspecting
the output alone.

### 14.1c Step 2's asymmetry premise is wrong as observed

The phase text says brink→winkel runs over native IPv6 (overhead 80) and
winkel→brink over DS-Lite CGNAT IPv4 (60, plus the AFTR's 40-byte v4-in-v6
tunnel), requiring 1440 and 1460 on the physical path respectively. **Live,
every WAN leg is direct over native IPv6** — the endpoints are
`[2a00:6020:b444:bb00:…]:41641` (Brink), `[2a00:6020:b481:e300:…]:41641`
(Winkel) and `[2a02:2479:5c:a00::1]:41641` (ionos). The only IPv4 endpoint in
the tailnet is maxdata→winkel-pi `192.168.178.3:41641`, which is same-LAN at
Winkel and crosses no WAN. So the overhead is a **uniform 80**, and the
1460-byte case the text worries about does not arise ~~and the overhead is a
uniform 80~~ — **in today's steady state only, and that qualification turned
out to matter more than the finding.**

⚠️⚠️ **The DS-Lite IPv4 leg is not hypothetical; it is one uplink flap away,
and it was measured by accident.** The first attempt at forcing DERP dropped
only IPv6 UDP to brink-server — and Tailscale did not fall back to a relay at
all. It reconnected **directly over IPv4**, NAT-traversed through Brink's
DS-Lite CGNAT at `94.31.94.151:7767`, and stayed there for a full minute of
polling. So the original step-2 premise describes the **fallback** path
correctly, and calling it inapplicable was hasty. Forcing an actual relay
required blocking **both** families.

**The fallback passes, but with no headroom at all**, and this is the tightest
number in the phase:

```
overlay 1400 + 32 WG + 8 UDP + 20 IPv4       = 1460
             + 40 DS-Lite v4-in-v6 to the AFTR = 1500  ← exactly the physical MTU
```

Measured: 0% loss at DF 1400 and a byte-exact 20 MB at 280 Mbit/s. It works —
**and it works at precisely the limit**, with zero bytes spare. The same
overlay MTU over IPv6 sits at 1480 and has 20 to spare.

⚠️ **Therefore prefer `overlay.mtu = 1380` over 1400.** It puts flannel at
1310 — still clear of the IPv6 floor of 1280 — while leaving **20 bytes of
margin on both underlay families** rather than 20 on one and none on the other.
1400 is not wrong today; it is a value that depends on every hop staying at
exactly 1500 forever, on the path that only appears when something has already
gone wrong. The failure it would produce is D3's blackhole, on a fallback,
during an incident — the worst possible time to be debugging MTU.
...
✅ **Read off the live tailnet 2026-08-12** with `tailscale ip -6` on each node:

| host | `overlayIPv4` | `overlayIPv6` |
|---|---|---|
| ionos | `100.64.0.1` | `fd7a:115c:a1e0::1` |
| brink-server | `100.64.0.2` | `fd7a:115c:a1e0::2` |
| winkel-pi | `100.64.0.3` | `fd7a:115c:a1e0::3` |
| maxdata | `100.64.0.5` | `fd7a:115c:a1e0::5` |

⚠️ **They do map to the v4 last octet, and that is exactly the trap this section
warned about** — the mapping being predictable is what makes deriving it feel
safe. It is still the control server's business: maxdata is `.5` and not `.4`
because the iPhone holds `100.64.0.4`, so even the "obvious" sequence already
has a hole in it that only enrolment order explains. Two nodes not in the
cluster share the prefix — iphone `100.64.0.4` and maxs-macbook-pro
`100.64.0.6`, both offline at the time of reading.
...
### 14.4a Checked 2026-08-12 — and the suspicion was right

Both hosts run **k3s v1.35.6+k3s1**.

| flag | `k3s server --help` | `k3s agent --help` |
|---|---|---|
| `--flannel-conf` | ✅ present, `(agent/networking)` | ✅ present |
| `--flannel-ipv6-masq` | ✅ present, `(networking)` | ❌ **absent** |

⚠️⚠️ **This is a live defect in the code that landed on 2026-08-12, and it would
have stopped k3s on winkel-pi.** `--flannel-ipv6-masq` sits in the
`lib.optionals dual` block at `modules/system/k3s-cluster.nix:292`, which is
evaluated for **every** node — it is above the `lib.optionals isServer` block,
not inside it. winkel-pi is the only agent, so enabling `cluster.dualStack`
would hand an agent a flag its CLI does not define, and k3s refuses to start
rather than warning. The flag's `(networking)` marking versus `--flannel-conf`'s
`(agent/networking)` is the distinction that decides this, and it is visible
only in the help output. **The code's own comment asked for this check; the
check fails.** Nothing was deployed, because `dualStack` defaults `false`.

✅ **Fixed the same day, and the fix is a one-line move after all.** The flag now
sits in a `lib.optionals (dual && isServer)` block; `--flannel-conf` stays on
every node.

⚠️ ~~Do not "fix" it by moving the flag into the `isServer` block: winkel-pi
runs pods, so server-only placement would leave its pods egressing from a ULA
with no return path — a build that succeeds and a behaviour that is wrong,
which is worse than the crash.~~ — **that caution was wrong, and it was written
here before being checked.** k3s's own networking documentation settles it in
two statements that have to be read *together*: the Flannel *cluster* options
(`--flannel-backend`, `--flannel-ipv6-masq`, `--flannel-external-ip`) "can only
be set on server nodes, and must be identical on all servers in the cluster",
while a **separate** *Flannel Agent Options* table lists exactly
`--flannel-iface`, `--flannel-conf` and `--flannel-cni-conf` as per-node. So
masquerading is cluster-scoped and **agents inherit it from the servers** —
winkel-pi's pods are masqueraded without the flag ever reaching them. The two
flags are not the same kind of thing, which is precisely what the identical
`--flannel-*` prefix hides.

⚠️ **Generalise, because this one nearly cost a wrong design rather than a
wrong command:** the plausible inference — "the flag is per-node, so a node
without it lacks the behaviour" — was reasoned from the flag's *name* and from
where it sat in the file. Neither is evidence. `k3s <sub> --help` says whether
a flag is *accepted*; only the documentation says what its **scope** is, and
an unrecognised-flag crash and a silently-unapplied setting are
indistinguishable from the Nix source. **Check scope, not just existence.**

### 14.4b Verified after the fix (2026-08-12)

Evaluated with `dualStack = true` and `overlay.mtu = 1400` forced in via
`extendModules`, which is the only way to see the dual-stack flag set without
committing to it:

| host | role | `--flannel-conf` | `--flannel-ipv6-masq` | `--node-ip` |
|---|---|---|---|---|
| ionos | server | ✅ | ✅ | `100.64.0.1,fd7a:115c:a1e0::1` |
| brink-server | server | ✅ | ✅ | `100.64.0.2,fd7a:115c:a1e0::2` |
| maxdata | server | ✅ | ✅ | `100.64.0.5,fd7a:115c:a1e0::5` |
| **winkel-pi** | **agent** | ✅ | **absent — correct** | `100.64.0.3,fd7a:115c:a1e0::3` |

Still inert at defaults: with `dualStack = false` all four carry no flannel
flag but `--flannel-iface`, a single-family `--node-ip`, and single-family
CIDRs — matching what is running today.

⚠️ **winkel-pi's `flannel-net-conf.json` is a different store path from the
servers' (`qn92wiy…` vs `i008wqx…`), and that is benign — but check it the
right way.** The cause is D12: winkel-pi builds against nixpkgs 26.05 and the
servers against 26.11, so `writeText` produces a differently-named derivation
for identical content. **Verified by comparing the content**, not the paths —
all four evaluate to byte-identical JSON, `Backend.MTU` 1380, which puts
flannel's devices and pod veths at 1330 and clears the 1280 floor with 50 to
spare. The hazard the file's own comment names — a `net-conf.json` disagreeing
with `--cluster-cidr` — would be invisible in a path comparison either way,
since the paths differ regardless.

⚠️ **`nix eval` of `services.k3s.extraFlags` does not run the assertions.**
Assertions are only forced when the system is built, so the guards added in
14.2 — the MTU arithmetic and the missing-`overlayIPv6` message — stay silent
here. The first attempt at this evaluation failed with `cannot coerce null to a
string` from `--node-ip` interpolating an unset `overlayIPv6`, which is the raw
version of the error the assertion exists to replace. Do not read a clean
`extraFlags` eval as the assertions having passed.

⚠️ **A deploy of this branch is not a no-op on ionos even with `dualStack`
off.** The `lanInterface` fix means ionos now receives `--cluster-cidr`,
`--service-cidr` and both `--etcd-arg`s for the first time. The CIDRs equal
k3s's defaults so nothing moves, but the etcd tuning is genuinely new there and
k3s restarts to pick it up — on the node that must never be built locally.
...
own would produce D3's blackhole rather than avoid it.** The reasoning is worth
following, because every individual step looks safe.

`overlay-mtu.service` is new on every host, so activation sets `tailscale0` to
1380 everywhere. But **k3s's flags changed on ionos alone** — that is the
`lanInterface` fix, which finally gives it `--cluster-cidr`, `--service-cidr`
and the two `--etcd-arg`s. So k3s restarts on **ionos only**, and flannel reads
its interface MTU at startup:

| node | `tailscale0` after deploy | k3s restarts? | resulting `flannel.1` |
|---|---|---|---|
| ionos | 1380 | **yes** (new flags) | **1330** |
| brink-server | 1380 | no | 1230 |
| maxdata | 1380 | no | 1230 |
| winkel-pi | 1380 | no | 1230 |

⚠️ **A 100-byte pod-MTU asymmetry across the cluster is exactly D3.** Pods on
ionos would emit up to 1330 while every receiving node's `flannel.1` and `cni0`
sit at 1230, so full-size pod-to-pod packets from ionos are dropped at the far
end while ping and every small request keep working. It is also **latent**:
existing pods keep the old CNI MTU until they are recreated, so it would appear
gradually, at rollout times unrelated to the change.

⚠️ **There is a second, narrower window during the rollout itself.** While some
hosts are at 1380 and others still at 1280, host-level TCP over the overlay —
etcd peer traffic and the apiserver — can blackhole in one direction: the
sender's larger segments exceed the receiver's TUN MTU, and a TUN drop does not
reliably generate the ICMP that path-MTU discovery needs. Pod traffic is *not*
exposed to this one, because VXLAN caps it at `1230 + 50 = 1280`, which both
ends accept.

**So the MTU change is not independently deployable.** Two ways to sequence it:

1. ✅ **Preferred — deploy it as part of 14.5.** The rebuild resets every node
   and recreates flannel uniformly, so the asymmetry cannot arise and the
   rollout window closes with the cluster already down. This is the default,
   and it is why the branch stays undeployed.
2. If it must go out sooner, raise `tailscale0` on **all four hosts by hand
   first** so activation is a no-op for the MTU, then deploy, then **restart
   k3s on every node** so flannel is uniformly 1330. The manual raise is what
   removes the rollout window; the fleet-wide k3s restart is what removes the
   asymmetry. Neither half is optional.

⚠️ Note this also means the ionos `lanInterface` fix — a genuine live
improvement, and the etcd tuning that node has never had — **cannot be shipped
on its own from this branch either**, since it is what triggers the very
...
- AAAA split-horizon records reopen three known traps at once: the `enabled: true`
  ...
  **The baseline to compare against, measured 2026-08-12** — query each site's
  AdGuard on its own `127.0.0.1`, which sidesteps the UDM SE's interception:

  | query | Winkel (winkel-pi) | Brink (brink-server) |
  |---|---|---|
  | `grafana.mvissing.de A` | `192.168.178.240` | `192.168.1.240` |
  | `grafana.mvissing.de AAAA` | **NOERROR / no answer** | **NOERROR / no answer** |
  | `mvissing.de A` | `192.168.178.240` | `192.168.1.240` |
  | `mvissing.de AAAA` | **NOERROR / no answer** | **NOERROR / no answer** |

  ⚠️ **That NODATA is doing real work and must not be lost by accident.** It is
  the `site-dns.nix:81-86` effect — an address rewrite suppresses other types
  for the name — and here it is protective: if AAAA fell through to the
  upstream, every internal client would receive ionos's public
  `2a02:2479:5c:a00::1` and hairpin out to the WAN and back for a service on
  the same LAN. That is the Phase 9 apex/CNAME failure with the family
  swapped. So the v6 work does not start from "no records", it starts from
  "records deliberately absent", and each AAAA added must point somewhere
  correct *before* the suppression is lifted.

- **LAN-side v6 exposure — the shape of it.** A second rewrite on the same
  wildcard carrying an address from `sites.<x>.ulaPrefix`
  (`fd06:f10a:ebec:178::/64` at Winkel, `fd06:f10a:ebec:1::/64` at Brink),
  answered by a MetalLB v6 pool from that same prefix. ⚠️ **The VIP must sit
  inside the on-link /64**, for exactly the reason the v4 VIP sits inside
  `192.168.178.0/24`: the hosts hold RA-derived addresses from it, so it is
  on-link and MetalLB's **NDP** announcement (not ARP, for v6) resolves.
  `externalTrafficPolicy: Local` keeps each site announcing only its own, as
  for v4. ⚠️ **Cross-site is not symmetric with v4 and is unresolved**: a Brink
  client reaching Winkel's v6 VIP needs the subnet router to advertise the v6
  prefix over the overlay, and today's advertisements are the IPv4 LAN
  prefixes only. Settle that before assuming the v4 topology carries over.

- ⚠️ **Be honest about what this buys.** With A and a ULA AAAA both present,
  RFC 6724 makes the client choose IPv4 every time — Chrome, Firefox and glibc
  all implement it. The v6 VIP will be correct, announced, reachable, and
  unused outside `curl -6`. Making clients genuinely prefer v6 needs GUA VIPs
  (precedence 40, above IPv4), which is precisely what D2 forbids depending on.
  **This layer is capability, not a change in how traffic flows** — which is
  worth stating plainly, because "expose services on the LAN over IPv6" sounds
  like it changes something observable and does not.
...
- **Matter is deployed but has never commissioned a device.** *(Moved here from
  Phase 10's exit criteria, 2026-08-12.)* Not a defect and not blocking
  anything — there are simply no Matter devices yet. Recorded because the
  deployment **looks complete from every angle that is normally checked**:
  `matter-server` is `Running` on brink-server, the `matter` config entry is
  loaded in Home Assistant, and the fabric store exists on disk. What shows it
  empty is the *contents*: `/data/658005795383821879.json` holds only
  `vendor_info` with **zero** numeric node keys, and HA's device registry has
  **no `matter` identifier**. ⚠️ **When the first device arrives, that pair is
  also the verification** — commissioning is done when both gain an entry, not
  when the UI reports success. ⚠️ Matter requires IPv6 **on the LAN**, which
