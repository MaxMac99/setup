...
- [ ] A simulated ZFS degradation produces an actual notification, not a
      ...
      Alertmanager → ntfy pipeline rather than a new notifier. A live test
      (`zpool offline` one `tank` `raidz1-0` disk, zero data risk) was
      proposed and **declined** — open by choice, not oversight
